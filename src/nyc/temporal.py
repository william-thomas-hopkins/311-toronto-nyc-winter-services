...
def _hourly_count(ts: pd.Series) -> pd.Series:
    ...
    s = s.dt.floor("h")
    return s.value_counts().sort_index().asfreq("h", fill_value=0)
...
def analyze() -> None:
    ...
    # Little's Law: L ≈ λ * W (use rolling λ and rolling W from recent closures)
    # λ: rolling mean of arrivals per hour; W: rolling mean observed cycle time (hours) from recent closures
    # Build hourly cycle-time series for closed items
    ...
    # assign the closure's hour as the time the "service time" becomes known
    closed["closed_hour"] = closed["closed_date"].dt.floor("h")
    service_time_h = (
        closed.set_index("closed_hour")["ttc_hours"].sort_index().asfreq("h").fillna(method="ffill")
    )
    ...
    for window_h, lab in [(24 * 7, "7d"), (24 * 14, "14d")]:
        ...
        W = service_time_h.reindex(hourly.index).rolling(window_h, min_periods=1).mean()
        L_pred = lam * W
        hourly[f"L_pred_{lab}"] = L_pred
